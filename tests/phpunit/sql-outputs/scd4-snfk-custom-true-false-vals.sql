...
CREATE TABLE "last_state" AS
    SELECT
        ...
        -- Actual flag is set to "'T'". --
        'T' AS "is_actual",
        -- IsDeleted flag is set to "'F'". --
        'F' AS "is_deleted"
    FROM "input_table" input;

-- Previous state: Set actual flag to "'F'" in the previous version of the records. --
...
-- Detection: Deleted records have actual flag set to "'T'" --
...
CREATE TABLE "deleted_records" AS
    SELECT
        ...
        -- The snapshot date is set to now. --
        $CURRENT_DATE_TXT AS "snapshot_date",
        -- The actual flag is set to "'F'" ("keep_del_active" = false). --
        'F' AS "is_actual",
        -- IsDeleted flag is set to "'T'". --
        'T' AS "is_deleted"
    FROM "current_snapshot" snapshot
    LEFT JOIN "input_table" input ON snapshot."pk1" = input."Pk1" AND snapshot."pk2" = input."pk2"
    WHERE
        snapshot."is_actual" = 'T' AND
        input."Pk1" IS NULL;
